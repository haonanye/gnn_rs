    ...
    def _generate_enc_graph(self, rating_pairs, rating_values, add_support=False):
        ...
        if add_support:
            ...
            for r in self.possible_rating_values:
                ...
                if self._symm:#symmetric normalize constant
            ...
            graph.nodes['user'].data.update({'ci' : user_ci, 'cj' : user_cj})#user node indegree & outdegree
            graph.nodes['movie'].data.update({'ci' : movie_ci, 'cj' : movie_cj})# movie node indegree & outdegree
